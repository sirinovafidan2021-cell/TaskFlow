...
class TaskAssignmentService
{
    ...
    public function assign(Task $task, ?User $assignee, User $actor): Task
    {
        return DB::transaction(function () use ($task, $assignee, $actor): Task {
            if ($assignee && ! $this->members->isMember($task->project, $assignee)) throw new LogicException('The assignee must belong to the project.');
            $oldAssignee = $task->assignee;
            if ($oldAssignee?->id === $assignee?->id) return $task;
