...
class TaskController
{
    ...
    public function show(Task $task): View
    {
        ...
        return view('tasks::show', ['task' => $task->load(['project','creator','assignee','comments.user','attachments.uploader']), 'memberships' => $this->members->memberships($task->project), 'nextStatuses' => $this->statuses->availableStatuses($task, request()->user()), 'activities' => $this->activity->recentForTask($task), 'canViewActivity' => request()->user()->can('viewAny', \Spatie\Activitylog\Models\Activity::class)]);
    }
    public function edit(Task $task): View { $this->authorize('update', $task); return view('tasks::edit', compact('task')); }
    public function update(UpdateTaskRequest $request, Task $task): RedirectResponse { $this->authorize('update', $task); $this->taskService->update($task, UpdateTaskData::fromArray($request->validated()), $request->user()); return redirect()->route('tasks.show', $task)->with('success', 'Task updated successfully.'); }
    public function destroy(Task $task): RedirectResponse { $this->authorize('delete', $task); $this->taskService->delete($task, request()->user()); return redirect()->route('tasks.index')->with('success', 'Task deleted successfully.'); }
    public function assign(AssignTaskRequest $request, Task $task): RedirectResponse { $this->authorize('assign', $task); $this->assignments->assign($task->load('project'), $request->filled('assignee_id') ? User::query()->findOrFail($request->integer('assignee_id')) : null, $request->user()); return back()->with('success', 'Task assignment updated.'); }
    public function changeStatus(ChangeTaskStatusRequest $request, Task $task): RedirectResponse { $this->authorize('changeStatus', $task); $this->statuses->change($task->load('project'), TaskStatus::from($request->string('status')->toString()), $request->user()); return back()->with('success', 'Task status updated.'); }
}
