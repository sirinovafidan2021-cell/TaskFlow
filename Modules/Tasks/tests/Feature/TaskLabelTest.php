...
test('API exposes scoped label CRUD, task resource labels, sync and filtering', function (): void {
    ...
    $this->putJson('/api/v1/tasks/'.$task['id'].'/labels', ['label_ids' => [$labelId]])->assertOk()->assertJsonPath('data.labels.0.slug', 'platform'); $this->getJson('/api/v1/tasks?label_ids[]='.$labelId)->assertOk()->assertJsonFragment(['id' => $task['id']]); $this->deleteJson("/api/v1/projects/{$project->id}/labels/{$labelId}")->assertForbidden();
