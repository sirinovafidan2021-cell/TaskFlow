...
<form method="POST" action="{{ $editing ? route('tasks.update', $task) : route('tasks.store', $project) }}" class="space-y-7">
    ...
    <section class="border-t border-slate-100 pt-7">
        ...
        <div class="mt-5 grid gap-5 sm:grid-cols-2">
            <div><label for="type" class="mb-2 block text-sm font-semibold text-slate-700">Work type</label><select id="type" name="type" class="block w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm">@foreach($types ?? \Modules\Tasks\Enums\TaskType::cases() as $type)<option value="{{ $type->value }}" @selected(old('type', $task->type->value ?? 'task') === $type->value)>{{ ucfirst($type->value) }}</option>@endforeach</select>@error('type')<p class="mt-2 text-sm font-medium text-rose-600">{{ $message }}</p>@enderror</div>
            <div><label for="parent_id" class="mb-2 block text-sm font-semibold text-slate-700">Parent task <span class="font-normal text-slate-400">(subtask only)</span></label><select id="parent_id" name="parent_id" class="block w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm"><option value="">No parent</option>@foreach($parents ?? [] as $parent)<option value="{{ $parent->id }}" @selected(old('parent_id', $task->parent_id ?? null) == $parent->id)>{{ $parent->display_key }} · {{ $parent->title }}</option>@endforeach</select>@error('parent_id')<p class="mt-2 text-sm font-medium text-rose-600">{{ $message }}</p>@enderror</div>
