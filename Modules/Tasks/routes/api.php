...
Route::middleware('abilities:tasks:read')->group(function (): void {
    ...
    Route::get('/tasks/{task}/comments',[TaskCommentController::class,'index'])->name('tasks.comments.index'); Route::get('/tasks/{task}/media',[TaskAttachmentController::class,'index'])->name('tasks.media.index'); Route::get('/tasks/{task}/media/{media}/preview',[TaskAttachmentController::class,'preview'])->name('tasks.media.preview'); Route::get('/tasks/{task}/media/{media}/download',[TaskAttachmentController::class,'download'])->name('tasks.media.download');
});
Route::middleware('abilities:tasks:write')->group(function (): void {
    Route::post('/tasks',[TaskController::class,'store'])->name('tasks.store'); Route::put('/tasks/{task}',[TaskController::class,'update'])->name('tasks.update'); Route::put('/tasks/{task}/labels',[TaskController::class,'syncLabels'])->name('tasks.labels.sync'); Route::delete('/tasks/{task}',[TaskController::class,'destroy'])->name('tasks.destroy'); Route::patch('/tasks/{task}/assignee',[TaskController::class,'assign'])->name('tasks.assign'); Route::patch('/tasks/{task}/status',[TaskController::class,'changeStatus'])->name('tasks.status'); Route::patch('/tasks/{task}/rank',[TaskController::class,'reorder'])->name('tasks.reorder'); Route::post('/tasks/{task}/media',[TaskAttachmentController::class,'store'])->name('tasks.media.store'); Route::delete('/tasks/{task}/media/{media}',[TaskAttachmentController::class,'destroy'])->name('tasks.media.destroy');
